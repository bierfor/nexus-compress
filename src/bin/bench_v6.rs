...
fn main() {
    ...
    for path in &paths {
        ...
        // v5 raw
        let (v5_compressed, v5_ms) = timed(|| {
            engine::compress_with("v5", &data, false).expect("v5 compress")
        });

        // v5 + conservative minify
        let (v5m_compressed, v5m_ms) = timed(|| {
        ...
        let (v6_compressed, v6_ms) = timed(|| {
            engine::compress_v6(&data, Some(&ext), 6)
        });
        let (v6e_compressed, v6e_ms) = timed(|| {
            engine::compress_v6(&data, Some(&ext), 9)
        });
    ...
    for r in &rows {
        ...
        let sevenz = if r.sevenz_size > 0 { format!("{:.2}x", ratio(r.size, r.sevenz_size)) } else { "n/a".to_string() };
        // category
    ...
    for r in &rows {
        ...
        let tag = if is_js_family(&r.ext) { "swc" } else { "conservative" };
    ...
    println!("|---|---:|---:|---:|---:|---:|---:|");
    for r in &rows {
        let lz = if r.sevenz_ms > 0.0 { format!("{:.1}", r.sevenz_ms) } else { "n/a".to_string() };
        println!(
            "| {} | {:.1} | {:.1} | {:.1} | {:.1} | {:.1} | {} |",
    ...
    println!("|---|---:|---:|---:|");
    let sevenz_ratio = if total_sevenz > 0 { total_size as f64 / total_sevenz as f64 } else { 0.0 };
    for (name, total) in [
        ...
        let r = if total > 0 { total_size as f64 / total as f64 } else { 0.0 };
        let pct = if total_sevenz > 0 && total > 0 {
            format!("{:.0}%", (r / sevenz_ratio) * 100.0)
        } else {
            "n/a".to_string()
        };
        let rs = if total > 0 { format!("{:.2}x", r) } else { "n/a".to_string() };
        let ts = if total > 0 { format!("{} B", total) } else { "n/a".to_string() };
    ...
    if total_sevenz > 0 && total_v6 > 0 {
        ...
        let mut cats: std::collections::BTreeMap<&str, (usize, usize, usize, usize, usize)> = std::collections::BTreeMap::new();
        for r in &rows {
            let cat = if r.file.contains("main-app") || (r.size > 1_000_000 && r.ext == "js") {
                "minified_bundle"
            } else if is_js_family(&r.ext) {
        ...
        for (cat, (s, v5t, v5mt, v6t, szt)) in &cats {
            ...
            println!("| {} | {:.2}x | {:.2}x | {:.2}x | {} |", cat, v5r, v5mr, v6r, diff_s);
        }
